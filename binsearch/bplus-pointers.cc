...
// potentially faster with hugepages
// also don't need for lower layers, but its hard to change the code
unsigned pointers[offset(H) / B][B + 1];
int *btree;
...
void prepare(int *a, int n) {
    ...
    for (int h = H - 1; h > 0; h--)
        for (int k = 0; k < (offset(h + 1) - offset(h)) / B; k++)
            for (int i = 0; i < 17; i++)
                // btree + offset(h) + k
                // k = k * (B + 1) + (i << 4)
                pointers[offset(h) / B + k][i] = offset(h - 1) / B + k * (B + 1) + i;
}
...
int lower_bound(int _x) {
    unsigned k = offset(H - 1) / B;
    reg x = _mm256_set1_epi32(_x - 1);
    for (int h = H - 1; h > 0; h--) {
        //std::cout << _x << " " << k << std::endl;
        unsigned i = permuted_rank(x, btree + k * B);
        __builtin_prefetch(&pointers[k][0]);
        __builtin_prefetch(&pointers[k][B]);
        k = pointers[k][i];
    }
    unsigned i = direct_rank(x, btree + k * B);
    return btree[k * B + i];
}
