...
void transpose_int(int *a, int *b) {
    // todo: fix indices
    for (int i = 0; i < P; i += 8) {
        reg r1 = _mm256_load_si256((reg*) &a[0 * P + i]);
        reg r3 = _mm256_load_si256((reg*) &a[1 * P + i]);
        reg r2 = _mm256_load_si256((reg*) &a[2 * P + i]);
        reg r4 = _mm256_load_si256((reg*) &a[3 * P + i]);
        reg r5 = _mm256_load_si256((reg*) &a[4 * P + i]);
        reg r7 = _mm256_load_si256((reg*) &a[5 * P + i]);
        reg r6 = _mm256_load_si256((reg*) &a[6 * P + i]);
        ...
        _mm256_store_si256((reg*) &b[8 * i + 8], r2);
        _mm256_store_si256((reg*) &b[8 * i + 16], r3);
        _mm256_store_si256((reg*) &b[8 * i + 24], r4);
        _mm256_store_si256((reg*) &b[8 * i + 32], r5);
        _mm256_store_si256((reg*) &b[8 * i + 40], r6);
        _mm256_store_si256((reg*) &b[8 * i + 48], r7);
        _mm256_store_si256((reg*) &b[8 * i + 56], r8);
    }
}
...
alignas(64) int res[SIZE], tra[SIZE];
...
int simd_parse(int &_s) {
            ...
            for (int k = 0; k < 8; k++) {
                for (int l = 0; l < 2; l++) {
                    for (int p = 0; p < K; p++) {
                        ...
                        int idx = p * (SIZE / K) + i + j * 16 + k * 32 + l * 8;

                        reg c = _mm256_cvtepu8_epi32(
                            _mm_loadl_epi64((__m128i*) &b[idx])
                        );
                        ...
                        _mm256_store_si256((reg*) &res[idx], _mm256_and_si256(x[p], mask));
        ...
        for (int q = 0; q < 8; q++) {
            for (int i = 0; i < SIZE / K; i += 256) {
                for (int j = 0; j < 2; j++) {
                    for (int k = 0; k < 8; k++) {
                        for (int l = 0; l < 2; l++) {
                            int idx = p * (SIZE / K) + q + i + j * 16 + k * 32 + l * 8;
                            _s ^= res[idx];
                            //if (res[idx])
                            //    printf("%d %d\n", idx, res[idx]);
                        }
    ...
    */
    ...
    for (int i = 0; i < K; i++)
        transpose_int(res + i * (SIZE / K), tra + i * (SIZE / K));
    ...
    for (int i = 0; i < SIZE; i += 8) {
        reg x = _mm256_load_si256((reg*) &tra[i]);
        s = _mm256_xor_si256(s, x);
        //_s ^= res[i];
    }

    int buf[8];
    _mm256_storeu_si256((reg*) buf, s);
